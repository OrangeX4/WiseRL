...
class MCM_AWAC(OracleAWAC):
    ...
    def select_reward(self, batch, deterministic=False):
        ...
        if isinstance(self.network.reward, DeterministicActor):
            logprob = torch.sum((self.network.reward.sample(obs)[0] - action)**2, dim=-1, keepdim=True)
        elif isinstance(self.network.reward, GaussianActor):
            logprob = self.network.reward.evaluate(obs, action)[0]
        reward = logprob + self.norm_factor
    ...
    def pretrain_step(self, batches, step: int, total_steps: int) -> Dict:
        ...
        all_obs = self.network.encoder(all_obs)
        self.network.reward.train()
        if isinstance(self.network.reward, DeterministicActor):
            all_logprob = torch.sum((self.network.reward.sample(all_obs)[0] - all_action)**2, dim=-1, keepdim=True)
            all_entropy = torch.full_like(-torch.inf, all_logprob.shape)
        elif isinstance(self.network.reward, GaussianActor):
            all_logprob, info = self.network.reward.evaluate(all_obs, all_action, return_dist=True)
            all_entropy = info['dist'].entropy().sum(-1, keepdim=True)
        all_reward = all_logprob.unsqueeze(0) + self.norm_factor  # TODO: we dont need to add norm_factor here actually
        all_entropy = all_entropy.unsqueeze(0)
        r1, r2 = torch.chunk(all_reward, 2, dim=1)
        entropy_1, entropy_2 = torch.chunk(all_entropy, 2, dim=1)
        E = r1.shape[0]
        r1, r2 = r1.reshape(E, F_B, F_S, 1), r2.reshape(E, F_B, F_S, 1)
        entropy_1, entropy_2 = entropy_1.reshape(E, F_B, F_S, 1), entropy_2.reshape(E, F_B, F_S, 1)
        ...
        with torch.no_grad():
            ...
            win_entropy = torch.where(win_mask, entropy_2.squeeze(), entropy_1.squeeze())
            lose_entropy = torch.where(win_mask, entropy_1.squeeze(), entropy_2.squeeze())


        self.optim["reward"].zero_grad()
        (reward_loss + self.reward_reg * reg_loss + self.entropy_reg * entropy_loss).backward()
        self.optim["reward"].step()

        metrics = {
            "loss/reward_loss": reward_loss.item(),
            "loss/reward_reg_loss": reg_loss.item(),
            "misc/entropy_loss": entropy_loss.item(),
            "misc/reward_acc": reward_accuracy.item(),
            "misc/reward_mean": all_reward.mean().item(),
            "misc/reward_std": all_reward.std().item(),
            "misc/entropy": all_entropy.mean().item(),
            "misc/win_reward_mean": win_reward.mean().item(),
            "misc/win_reward_std": win_reward.std().item(),
            "misc/win_entropy": win_entropy.mean().item(),
            "misc/lose_reward_mean": lose_reward.mean().item(),
            "misc/lose_reward_std": lose_reward.std().item(),
            "misc/lose_entropy": lose_entropy.mean().item(),
        }
        return metrics
