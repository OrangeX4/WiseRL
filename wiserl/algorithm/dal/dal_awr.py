...
class DAL_AWR(Algorithm):
    ...
    def train_step(self, batches, step: int, total_steps: int) -> Dict:
        ...
        exp_advantage = (reward / self.beta).exp().clip(max=self.max_exp_clip)
        if isinstance(self.network.actor, DeterministicActor):
            policy_out = torch.sum((self.network.actor.sample(obs.detach())[0] - action)**2, dim=-1, keepdim=True)
        elif isinstance(self.network.actor, GaussianActor):
            policy_out = - self.network.actor.evaluate(obs.detach(), action)[0]
